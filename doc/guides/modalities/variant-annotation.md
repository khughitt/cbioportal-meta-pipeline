...
- Tate JG, et al. 2019. "COSMIC: the Catalogue Of Somatic Mutations In Cancer." *Nucleic Acids
  Res* 47:D941–D947. PMID 30371878. → Cancer Gene Census Tier 1 + Tier 2.
...
| annot.05 | Oncotator canonical-isoform filter (applied upstream by cBioPortal) is documented as a known restriction | any aggregation that depends on per-variant calls | settled | script header notes that non-canonical-isoform-only mutations are silently excluded from cBioPortal-fed summaries (per Cerami 2012 / Gao 2013) |
| annot.06 | Hotspot annotations from both 1D (Chang 2016) and 3D (Gao 2017) catalogs applied | per-mutation outputs | contested | both `is_1d_hotspot` and `is_3d_cluster_residue` columns; the two are largely disjoint (~3% overlap per Gao 2017) and both add value |
| annot.07 | Catalog refresh cadence documented | any pipeline using a versioned catalog | settled | per-catalog refresh cadence (OncoKB monthly; CGC per-release; Bailey 2018 static; Hotspots static) noted in pipeline docs |
...
- **Treating OncoKB-Level-1 as time-invariant.** Per Suehnholz 2024, the Level 1 fraction of a
