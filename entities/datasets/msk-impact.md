---
type: dataset
title: MSK-IMPACT clinical sequencing cohort
status: active
created: '2026-05-30'
updated: '2026-06-27'
id: dataset:msk-impact
source_class: observational
dataset_class: deposit
local_path: /data/raw/cbioportal/msk_impact_2017
source_refs:
- paper:Bandlamudi2026
- paper:Cheng2015
- paper:Suehnholz2024
- paper:Zehir2017
- paper:Mina2020
- paper:Schill2024
related: []
---

# MSK-IMPACT clinical sequencing cohort

## Summary

Memorial Sloan Kettering Integrated Mutation Profiling of Actionable Cancer Targets (MSK-IMPACT) — large targeted-panel (341/410/468-gene) clinical tumor-sequencing cohort with matched normals. Includes successive releases [@Zehir2017; @Bandlamudi2026] and the MSK-IMPACT samples distributed via AACR GENIE.

## Access and Scope

- Accessions: cBioPortal msk_impact_2017; AACR GENIE
- Source URL: https://www.cbioportal.org/study/summary?id=msk_impact_2017
- Organism/population: Homo sapiens
- Modality: targeted-panel WES (matched normal)
- Sample size: ~10k–50k tumors across releases

## Connections to Project

- Referenced by: `paper:Bandlamudi2026`, `paper:Cheng2015`, `paper:Suehnholz2024`, `paper:Zehir2017`, `paper:Mina2020`, `paper:Schill2024`
- Role: external dataset cited in the project literature; see referencing papers for usage.
