...
length [@Lawrence2014; @Martincorena2017; @Bailey2018] — but no comparable correction exists
...
PTMs, higher PPI degree, more "things to write about" per gene). Stoeger et al. [@Stoeger2018]
...
- **P2 (direct path exists, central claim).** The partial slope `β_length` in
  ...
  the human-cancer-relevant subset (CGC + Bailey et al. [@Bailey2018]) yielding a slope of similar sign and
  magnitude.
...
- The slope `β_length` differs across **time windows**: in older data (1990s–2000s
  ...
  replication of Stoeger et al. [@Stoeger2018] over time periods is a natural extension.
...
- **Lawrence et al. [@Lawrence2014]:** establishes the gene-length-mediated mutation-count bias that
  forms the mediator path.
- **Gillis and Pavlidis [@Gillis2013]:** prior literature on gene-popularity confounds.
...
- **Decisive test:** Run the partial-slope regression (`t129`, scoped in
  `discussion:0001-gene-length-bias-in-mutation-rankings-and-literature`) on the full
  pan-cancer table after t144/t145 propagate. Report `β_length` with bootstrap 95% CI on:
  (a) all protein-coding genes, (b) CGC subset, (c) Bailey et al. [@Bailey2018] subset, (d) 2010+
...
- **Sibling hypotheses:** `hypothesis:0002-cross-study-ranking-divergence-is-structured` (the
  ...
  `hypothesis:0001-non-tumor-signal-contamination` (the data-quality partner — what mutation-count is
  *actually* measuring).
- **External:** Stoeger et al. [@Stoeger2018]; Gillis and Pavlidis [@Gillis2013];
  Edwards et al. [@Edwards2011] ("Too many roads not taken").
