...
1. The pipeline already emits a `ch_priority_gene` boolean via `annotate_ch.py` for the
   Bolton et al. [@Bolton2020] 7-gene list (DNMT3A, PPM1D, TET2, TP53, ASXL1, CHEK2, PRPF8). This is a
   *uniform* flag — but the literature shows the CH-vs-tumor probability varies
   ...
   Coombs et al. [@Coombs2018]). ASXL1 and TET2 sit in between, and their status depends on cancer type
...
Scope includes: matched-normal vs tumor-only quantitative studies (Coombs et al. [@Coombs2018],
Ptashkin et al. [@Ptashkin2018], Bolton et al. [@Bolton2020]); solid-tumor ASXL1/TET2 biology (CRC MSI, CRPC, HNSCC,
...
| 8 | Cai X, Bowman RL, Trowbridge JJ. "Clonal hematopoiesis in myeloid malignancies and solid tumors." *Nat Cancer* [@Cai2025ClonalHematopo] | 2025 | PMID: not confirmed in this note / DOI: 10.1038/s43018-025-01014-0 | Relevant next | Current synthesis of CH in both contexts. Likely the freshest authoritative reference for future topic expansion. |
| 9 | Vlasschaert C, Mack T, Heimlich JB, et al. "A practical approach to curate clonal hematopoiesis of indeterminate potential in human genetic data sets." *Blood* [@Vlasschaert2023PracticaApproach] | 2023 | PMID: not confirmed in this note / DOI: 10.1182/blood.2022018825 | Relevant next | **CHIP curation framework.** Stepwise sequencing-metric / variant-annotation / population-frequency filters. Most directly actionable methods paper for tightening our `annotate_ch.py` beyond the 7-gene boolean flag. |
| 10 | Roerink SF, ..., Petljak M. "ArCH: improving the performance of clonal hematopoiesis variant calling and interpretation." *Bioinformatics* [@Roerink2024ArCH] | 2024 | PMID: 38485690 / DOI: 10.1093/bioinformatics/btae121 | Relevant next | **ArCH** — artifact-filtering CH variant calling pipeline. Combines 4 variant callers + sequencing-error-rate estimation. Ready-to-use open-source tool (if we extend the pipeline to BAM-level CH filtering, which we currently don't). |
| 11 | ASXL1 neomorphic-mutation mechanistic report | 2021 | DOI: 10.1007/s13238-020-00799-3 / PMID: not confirmed in this note | Relevant next | Mechanistic reference for **solid-tumor ASXL1 neomorphic mutations being functionally distinct from CH-driver ASXL1 mutations**. Supports variant-level (not just gene-level) disambiguation — a potential future refinement of `annotate_ch.py`. |
...
| 13 | Zehir A, et al. "Mutational landscape of metastatic cancer revealed from prospective clinical sequencing of 10,000 patients." *Nat Med* [@Zehir2017] | 2017 | PMID: 28481359 / DOI: 10.1038/nm.4333 | Relevant next | Already in project bib. The MSK-IMPACT matched-buffy-coat design reference; quantifies baseline matched-normal fraction for one of our major ingested studies. |
| 14 | Ptashkin RN, Ewalt MD, Jayakumaran G, et al. "Enhanced clinical assessment of hematologic malignancies through routine paired tumor and normal sequencing." *Nat Commun* [@Ptashkin2023EnhancedClinical] | 2023 | DOI: 10.1038/s41467-023-42585-9 / PMID: not confirmed in this note | Relevant next | Demonstrates paired-normal retention of >10,000 germline + CH variants in a routine-clinical setting. Shows the method works operationally at scale. |
| 15 | FoundationOne-specific tumor-only CH filtering report | 2018 | PMID: 30348652 / DOI: 10.1182/blood-2018-05-849059 (identifier pairing not reconfirmed in this note) | Relevant next | FoundationOne-specific tumor-only CH prevalence estimation. Complements Coombs et al. [@Coombs2018] (also FoundationOne) with variant-characteristic filters. |
...
1. **Coombs et al. [@Coombs2018] — 8% variant-level CH confirmation rate, per-gene stratification** — the
...
2. **Ptashkin et al. [@Ptashkin2018] — 5% patient-level misattribution in MSK-IMPACT** — read second.
   Complementary (patient-level vs variant-level) to Coombs et al. [@Coombs2018] on the same underlying
   phenomenon.
3. **Bolton et al. [@Bolton2020]** — read third if not already; establishes the 7-gene priority list our
   pipeline currently uses.
4. **Coombs et al. [@Coombs2017]** — read for the foundational framing of CH in solid-tumor cohorts.
...
8. Cai et al. [@Cai2025ClonalHematopo] Nat Cancer review — current cross-context CH synthesis.
9. Reich 2023 CHIP curation framework — actionable sequencing-metric filters.
10. Roerink et al. [@Roerink2024ArCH] ArCH — open-source CH variant caller.
11. 2021 ASXL1 neomorphic-mutation report — tumor-derived *neomorphic* ASXL1 mutations (variant-level biology).
12. Magee et al. [@Magee2025PlasmaCfDNA] — cfDNA-level CH-vs-tumor disambiguation.
13. Zehir et al. [@Zehir2017] — MSK-IMPACT matched-buffy-coat baseline.
14. Ptashkin et al. [@Ptashkin2023EnhancedClinical] — paired-normal operational scale evidence.
...
- **Variant-level disambiguation in tumor-only data is operationally unsolved for ASXL1.**
  Coombs et al. [@Coombs2018] achieves its 8% number by going paired; ArCH and similar tools improve on
...
- **MSK-IMPACT Heme vs MSK-IMPACT.** Ptashkin et al. [@Ptashkin2023EnhancedClinical] documents that MSK-IMPACT Heme is the
...
1. **Update `annotate_ch.py` annotations** with per-gene CH-contamination-fraction
   estimates from Coombs et al. [@Coombs2018] (variant-level, per-gene: DNMT3A ≈ 64%, TP53 ≈ 4%, ASXL1
   and TET2 intermediate — extract the exact numbers from Table 2 of Coombs et al. [@Coombs2018]). This
...
3. **Paper stubs for the 6 Core-now entries** at `doc/background/papers/` — Coombs et al. [@Coombs2018],
   Ptashkin et al. [@Ptashkin2018], Bolton et al. [@Bolton2020] (already exists), Coombs et al.
   [@Coombs2017], Katsuoka/Katoh [@Katsuoka2013], Stampone et al. [@Stampone2025].
...
7. **BibTeX entries** for the 6 Core-now entries (now present in bib: Bolton et al.
   [@Bolton2020], Zehir et al. [@Zehir2017], Coombs et al. [@Coombs2018], Ptashkin et al.
   [@Ptashkin2018], Coombs et al. [@Coombs2017], Katsuoka/Katoh [@Katsuoka2013], and
   Stampone et al. [@Stampone2025]).
