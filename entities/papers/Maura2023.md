...
| APOBEC activity (SBS2+SBS13) → short PFS | APOBEC positive-control axis (APOBEC3 expression ↔ SBS2/13) | Direct ground-truth case for hypothesis:0007-agnostic-covariate-association-recovers-known-signature-aetiologies-and recovery; paper validates outcome correlation |
| SBS9 (germinal center polymerase eta, low = worse PFS) | Novel candidate for discovery-arm scan | MM-specific; not a standard "known" COSMIC exposure–aetiology pair |
...
| Immune microenvironment (NK, T, monocyte) ↔ treatment response | Expression-module covariate axis | Demonstrates single-cell immune expression covariates predict signature-outcome axes |
| XBP1/IKZF3/RPL5 driver annotation | Bailey et al. [@Bailey2018] driver overlay + custom annotations | These myeloma-specific drivers could extend the pipeline's annotate_drivers overlay |
...
- **mmsig tool:** Examine for compatibility with the cbioportal pipeline's restricted signature assignment step (question:0018-can-mutational-signature-decomposition-be-added-downstream-of-the-cross). The mmsig fitting algorithm is validated here on MM WGS and available on GitHub.
- **APOBEC validation in cBioPortal MM studies:** Check whether APOBEC signal is reproducibly detected across cBioPortal MM studies in the cross-study aggregation. This would validate positive-control recovery in a cross-study (rather than single-trial) setting.
- **RPL5 / XBP1 / IKZF3 as driver annotations:** Consider adding these to the pipeline's driver gene list (currently anchored to Bailey et al. [@Bailey2018]), especially for myeloma-focused analyses.
- **SBS9 in other cancer types:** The germinal center hypermutation signature is active primarily in B-cell neoplasms. In a cross-cancer scan, it provides a B-cell lineage specificity test for discovery-arm results.
- **Immune-signature crosstalk:** The paper hints but does not fully execute a joint immune-expression / mutational-signature association scan. This is precisely the discovery-arm design gap.
