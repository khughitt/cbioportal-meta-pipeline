...
- **Per-platform vs joint integration.** Two main approaches:
  ...
  - *Joint multivariate latent-variable models* (iCluster, MOFA, JIVE) — harder, but exposes
    cross-platform patterns. Hoadley et al. [@Hoadley2018] use iCluster across CNA + methylation +
    mRNA + miRNA on 9,759 TCGA tumors.
- **Variance contributions.** Hoadley et al. [@Hoadley2018]: CNA ~47%, transcriptome (mRNA +
  miRNA) ~42%,
  methylation ~11% in their iCluster decomposition. CNA dominates the multi-omic signal.
- **Mutations are typically excluded from multi-omic clustering due to sparsity** (Hoadley et al.
  [@Hoadley2018]
...
- **Mutation + outcome integration.** Jee et al. [@Jee2024] (MSK-CHORD) integrate MSK-IMPACT mutation calls
...
- **Pathway-level integration** (Sanchez-Vega et al. [@SanchezVega2018]) collapses mutation + CNA +
  fusion + methylation
...
Hoadley et al. [@Hoadley2018], Sanchez-Vega et al. [@SanchezVega2018], and Bailey et al.
[@Bailey2018] form a complementary tri-axis on the same ~10,000 tumor cohort: iCluster taxonomy,
pathway view, and driver-gene catalog.

For non-TCGA / panel-based cohorts, multi-omic integration is much sparser. MSK-CHORD
(Jee et al. [@Jee2024]) adds clinical outcomes; Bandlamudi et al. [@Bandlamudi2026] add CCF /
clonality / HLA LOH per sample.
...
2. **Pathway-level overlay** (Sanchez-Vega et al. [@SanchezVega2018] Tables S2/S3) — pathway
   memberships for 10
