...
2. **High priority — CH list breadth.** `Poon2021`'s unexplained-selection finding argues directly that the Bolton 7-gene `ch_priority_gene` list is a floor, not a ceiling. Tracked as `question:0006`. Before expanding, we should ask whether an empirical inclusion criterion (e.g., "gene with excess synonymous burden in blood-adjacent tissues in a published CH panel") would serve better than a fixed list.

3. **Medium priority — hypothesis-level calibration.** `Xu2025`'s per-tissue, per-gene normal-tissue burden table is the closest thing to a "null model" for what a cross-study frequency of X% in tissue Y is expected to be under background mutagenesis alone. Attaching this as a background column to the `_annotated` feathers would let downstream analysis explicitly subtract (or flag) background-consistent frequencies. Tracked as `question:0007`.

4. **Medium priority — replication-timing confounder.** `Yaacov2023` means that replication-timing-stratified analyses are worth considering when comparing gene-level ranks across cancer types whose dominant signatures have different RT preferences (e.g., lung/SBS4 vs colon/SBS88). Tracked as `question:0003`.

5. **Lower priority (for now) — CNA/mCA contamination.** `Lin2024` and `Gao2023` flag contamination risks that do not affect current SNV outputs but do affect any future CNA aggregation. Tracked as `question:0002` (breast CNA specifically) and `question:0004` (mCA in esophagus). Keep these in mind before enabling CNA workflows.

6. **Site-specific inflation flags.** `question:0001` (esophageal NOTCH1 from Yoshida2026) and `question:0005` (GLI1 exon 12 hotspot from Xu2025) are narrow, testable predictions — both can be checked directly against current pipeline outputs with a `gene_cancer_study_ratio_annotated.feather` query.

7. **Signature-space contamination, not just gene-space.** See companion synthesis `topic:signature-decomposition-unmatched-normal`. The background tissue-of-origin signal these six papers describe does not only leak through as inflated gene-level frequencies — it also perturbs mutational-signature decomposition in any per-study analysis run on cBioPortal cohorts without matched normals. Tracked as `question:0008` (tissue-background subtraction feasibility), `question:0009` (SBS1 replication-timing bias as a contamination QC flag, per Yaacov2023), and `question:0010` (tissue-of-origin classifier to flag normal-like samples).
