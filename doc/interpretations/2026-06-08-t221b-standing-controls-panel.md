---
id: "interpretation:2026-06-08-t221b-standing-controls-panel"
type: "interpretation"
title: "t221(b): four standing controls lock down the candidate-set verdict — the residual is entirely an all-region call-set artifact (6 studies; gone in 84 exonic-clean ones), the candidates are statistically indistinguishable from known CFS genes, germline (dbSNP) leak is not the driver, and no label-free neural set reproduces the enrichment"
status: "active"
related:
...
supporting evidence for the **candidate-set mutational-count thread** of `h12`; `hypothesis:h12` (proposed)
and `question:q032` (active) are intentionally left in their prior states — one thread does not flip the
