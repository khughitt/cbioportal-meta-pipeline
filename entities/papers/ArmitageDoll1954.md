...
This distinction is directly relevant to `hypothesis:0006-pre-malignant-n-minus-1-driver-carriage`: while Armitage-Doll provides a theoretical warrant for expecting that pre-malignant cells carry n-1 prior hits, the cBioPortal data cannot directly observe those pre-malignant states. And `question:0012-mutation-ordering-cross-sectional-inference` inherits the same limitation — cross-sectional inferences about mutation order from diagnostic cohorts face the exact selection pressures that make Armitage-Doll predictions hard to test in our data.

The `theme:0003-temporal-structure-of-carcinogenesis-order-count-and-timing-of-the` is where the Armitage-Doll conceptual vocabulary (ordered stages, stage-specific hit rates, latent periods, variable vs. constant hit-rate environments) maps most directly onto the project's interpretive framework.
