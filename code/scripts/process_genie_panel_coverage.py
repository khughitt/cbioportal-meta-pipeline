...
# t070: normalize MSK-IMPACT panel names to project-canonical form (see below).
from resolve_panel_id import PANEL_ALIASES  # type: ignore[import-not-found]
...
# t070: canonicalize MSK-IMPACT panel names to the project-canonical form
# (GENIE uses "MSK-IMPACT341", we use "MSK-IMPACT-341"). Non-MSK panels pass
# through unchanged since downstream consumers don't need canonical form for
# panels we don't produce samples.panel_id for.
panel_id = raw["SEQ_ASSAY_ID"].astype(str).str.strip()
panel_id = panel_id.map(lambda p: PANEL_ALIASES.get(p, p))
