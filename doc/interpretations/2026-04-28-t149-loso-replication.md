...
The `pooled_rate` top-K is not the project's primary ranking metric, but the fact that it is this unstable is itself relevant to `question:q015-pan-cancer-aggregator-choice`: any aggregator that effectively reduces to "rank by point estimate" inherits this instability.
