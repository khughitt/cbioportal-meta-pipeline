...
across all seven `pre-registration:0003-h08-positive-control-agnostic-association-must-recover-known-signature` arm strata (SKCM, LUAD, LUSC, BLCA,
...
This is check 2 of `plan:0005-h08-positive-control-scan-analysis-plan` and a near-term unblocker for the
...
- **BRCA sub-floor fraction.** Only 22.5% of BRCA samples clear the matched-normal floor of 100
  (median 52 SBS/sample — consistent with the WES median cited in `question:0018`). Samples are
...
`question:0018` (check 1), both near-term unblockers for the H08a scan are closed. Remaining:
