...
created: '2026-06-01'
updated: '2026-06-28'
id: meta:0010-next-steps-2026-06-01
prior: meta:0009-next-steps-2026-05-31
related:
...
Project links: this planning snapshot centers
`hypothesis:0007-agnostic-covariate-association-recovers-known-signature-aetiologies-and` and the
active signature-association and treatment-contamination work queue, including `task:t195`,
`task:t199`, `task:t200`, and `task:t205`.
...
The repair document is `pre-registration:0004-h08-smoking-arm-repair-repaired-smoking-covariate-must-recover-sbs4`; the next executable step is a small implementation plan for the repaired Arm-B production rerun, not a broad `H08b` scan.
...
Under `pre-registration:0004-h08-smoking-arm-repair-repaired-smoking-covariate-must-recover-sbs4`, the local repaired Arm-B verdict is therefore `[?]`, because the rank <= 3 criterion fails.
...
| Prior recommendation | Current status | Evidence |
|---|---|---|
| Decide whether `H08b` can proceed after the repaired smoking result | Done | `discussion:0005-h08b-gate-handling` records that no confirmatory `H08b` gate is open; any `task:t182` work is exploratory/prototype only. |
