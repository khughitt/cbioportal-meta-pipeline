---
id: "search:2026-04-14-pathway-level-pan-cancer-methods"
type: "search"
...
data**, going beyond Sanchez-Vega 2018's 10-oncogenic-signaling-pathways treatment (which is
...
2. **Methods-anchor lookup** — named-method queries for: SLAPenrich (Iorio), ActivePathways
   ...
   NetBox (Cerami), TieDIE (Paull), PCAWG pathway-and-network (Reyna 2020), IntOGen
...
5. **Leiserson 2015 — HotNet2 pan-cancer** — read for the canonical pan-cancer
   network-method result; the diffusion framework is broadly applicable.
6. **Horn 2018 — NetSig** — read for the node-degree-bias correction, which is a concrete
   risk when layering any PPI prior on our gene-level counts.

**Relevant next:**

7. Reimand 2019 — g:Profiler / GSEA / EnrichmentMap protocol (baseline off-the-shelf).
8. Reyna 2018 — Hierarchical HotNet (nested subnetworks).
9. Hofree 2013 — Network-Based Stratification (patient clustering upgrade path).
10. Vaske 2010 — PARADIGM (when we add expression).
11. Cerami 2010 — NetBox (cBioPortal-ecosystem module discovery).
12. Vandin 2011 — HotNet1 (heat-diffusion framing).
13. Paull 2013 — TieDIE (multi-omics coupling).
14. Knijnenburg 2018 — Pan-cancer DDR landscape (pathway-level result exemplar).
15. Iranzo 2018 — Cancer-mutation network evolution.
16. Sondka 2018 — COSMIC Cancer Gene Census.
17. Wu 2022 — driver-method benchmark.

**Peripheral monitor:**

18. ICGC/TCGA PCAWG 2020 flagship (umbrella).
19. Reimand 2024 — DPM multi-omics directional pathway method.
...
- **Sanchez-Vega 2018's 10-pathway taxonomy is the default project seed** but has known
...
- **PMID verification gap.** Nine of the twenty cited PMIDs/DOIs are flagged
  `[UNVERIFIED]` — these came via LLM knowledge + title-match rather than direct
...
6. **Check CH-pathway contamination** as a new sub-question: when CH-priority genes
   (Bolton 2020 7-gene list) are allowed in the gene-level input, which pathway methods
