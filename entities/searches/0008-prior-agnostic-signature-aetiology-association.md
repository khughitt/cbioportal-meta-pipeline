...
1. **Expression-module covariates at scale.** The published covariate axes are clinical
   ...
   (`paper:Luo2023a`). **Unsupervised co-expression *modules* (NMF on mRNA) as the agnostic covariate
...
2. signature exposure × gene expression × APOBEC3A/B (SBS2/13) → `paper:Luo2023a`, APOBEC-expression
...
8. signatures × tumor microenvironment / immune → `paper:Luo2023a`.
