...
hypothesis asks the signature analog of `hypothesis:0002` (which concerns mutation-frequency ranking
...
- The literature batch shows signature *estimates* are tool- and reference-dependent
  ...
  reproducible artefactual signatures (`paper:Jiang2025a`). These are exactly the cross-study nuisances
  the project is positioned to measure, because it holds the same tissue sequenced under different
  pipelines.
- `hypothesis:0002` already establishes that cross-study *ranking* divergence is structured (not
...
2. Residual cross-study exposure variance is predicted by technical covariates — caller consensus
   (`paper:Jiang2025a`), assay class (panel/WES/WGS), and treatment-exposed fraction — more strongly
...
- **Reference-version skew** — studies fit against different COSMIC versions (`question:0021` heme
