...
Hotspots, 3D Hotspots), or by overlay against curated catalogs (Bailey 2018, CGC, OncoKB).
...
**Reference catalogs:**
- Bailey 2018 Table S1 — pan-cancer + per-cancer driver rosters with Gold/Silver/Bronze tiers.
- COSMIC Cancer Gene Census (Tate 2019) — 750 Tier-1 + Tier-2 genes.
- Cancer Hotspots (Chang 2016) — `github.com/taylor-lab/hotspots`.
- 3D Hotspots (Gao 2017) — `3dhotspots.org`.
...
| driver.03 | Bailey 2018 catalog overlay applied | per-(gene, cancer) ranking outputs | settled | `bailey2018_driver` boolean column present (per `code/scripts/annotate_drivers.py`); pan-cancer + per-cancer-type rosters distinguished |
...
| driver.05 | Hotspot overlay applied | per-mutation or per-(gene, cancer) outputs | contested | per-mutation `is_1d_hotspot` (Chang 2016) and `is_3d_cluster_residue` (Gao 2017); aggregable to per-gene "fraction in hotspots" |
| driver.06 | Saturation considerations cited for long-tail claims | per-cancer driver-discovery claim | settled | per-cancer required-N from Lawrence 2014 cited; cancer types below saturation flagged when a long-tail novel driver is reported |
| driver.07 | Catalog version stamped | any catalog overlay output | settled | catalog version (Bailey 2018 supp release date; CGC version; OncoKB snapshot) recorded per overlay column |
| driver.08 | Tissue-borrowed vs tissue-matched drivers distinguished | per-(gene, cancer) annotated outputs | contested | overlay distinguishes "Bailey driver in this cancer" vs "Bailey pan-cancer driver only" — surfaces the 19% tissue-borrowed phenomenon |
| driver.09 | dNdScv hypermutator exclusion handled | dNdScv output | settled | hypermutators excluded from cohort fit per Martincorena 2017 default (`max_coding_muts_per_sample=3000`); MMR / POLE samples handled with extended context model where applicable |
...
- **Equating recurrence rank with driver-ness.** Long genes (TTN ~3×TP53 length) appear at the
  ...
  family) or selection-based methods (dNdScv) catch this. Lawrence 2014: 93% of genes carry ≥5
...
- **Pan-cancer driver lists used as per-cancer references.** A gene in Bailey 2018's 299-gene
...
- **Single-tool "novel driver" claims.** Bailey 2018: union of 8 phase-1 tools = 2,101 genes;
