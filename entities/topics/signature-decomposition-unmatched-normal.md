...
**Caveat:** The Li 2021 list is from a very small cohort (5 donors, Eastern Asian). Confidence in gene-tissue risk assignments is moderate for major drivers (NOTCH1, TP53), lower for secondary genes. Track as `question:0008`.
...
- `question:0001`: NOTCH1 inflation in esophageal studies — direct application of normal-epithelial contamination in a specific tissue. The signature-decomposition framing adds the mechanistic layer: NOTCH1-mutant normal esophageal clones will carry SBS1/SBS5-dominated spectra; their contribution will inflate the SBS1 exposure of the affected study.
- `question:0003`: Replication-timing as a gene-level mutation-rate confounder — directly relevant to interpreting LRR/ERR-differential signatures in cancer vs normal.
