...
**count/timing** sibling of the project's **order** work (`question:0012`, `hypothesis:0004`,
`hypothesis:0006`) and `discussion:0007-hallmark-ordering-and-data-driven-modules`.
...
- **The killer confound is age -> mutation-detection circularity.** Older patients carry more
  ...
  and report whether the signal survives (`question:0041`).
...
- **`hypothesis:0006` (pre-malignant n-1 driver carriage)** is the closest existing home: it is
...
- **`question:0012` / `hypothesis:0004`** are the *order* axis; this discussion is the *count/timing*
  axis. Same cohort, same confounds (collider, CH, callability, Simpson).
- **`discussion:0007-hallmark-ordering-and-data-driven-modules`** is the hallmark-grain echo
...
**confound-controlled correlation** (`question:0041`); and (ii) timing may be governed by
