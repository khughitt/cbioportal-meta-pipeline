...
type: "interpretation"
title: "t218: the candidate mutation enrichment is not CNS-driven (q033) and the t217 residual is not panel ascertainment — it is one cohort's all-region (98.5%-intronic) mutation table tiling the candidates' multi-Mb loci; removing it returns them to the genomic-span null, closing the candidate-set mutational-count thread of h12"
status: "active"
related:
...
project entities `hypothesis:h12` (proposed) and `question:q033` (active) are intentionally left in their
...
3. **Fold the all-region-table finding into `q016` / `theme:assay-regime-confounding`.** A cohort whose
