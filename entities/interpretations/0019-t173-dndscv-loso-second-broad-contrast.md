...
The Snakemake dry-run resolved the isolated DAG. The real `task:t173` run completed all 330 dynamic steps,
...
The pooled holdout table contains 20,091 genes with non-null `min_qglobal` (`task:t173`).
...
0.852, and the minimum remains 0.429 because GENIE is the only low-overlap holdout (`task:t173`).
...
Genes lost from the base top 100 after excluding `pog570_bcgsc_2020` for `task:t173`:

`BRCA1, CIC, PTCH1, TCF3`

Genes gained in the exclude-POG top 100 for `task:t173`:
...
ranking, while GENIE removal substantially reshuffles it (`task:t173`).
