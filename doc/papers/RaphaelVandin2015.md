...
ontology_terms: []
datasets:
  - "Wood et al. 2007 colorectal cancer (95 samples, 8 genes)"
  - "TCGA colorectal cancer (224 samples, 14 genes; extended: 43 genes)"
  - "TCGA glioblastoma multiforme (251 samples, 27 genes)"
source_refs:
...
updated: "2026-06-07"
dataset_usage:
  - ref: "dataset:tcga-colorectal"
    role: "analyzed"
    overlap: "unknown"
  - ref: "dataset:tcga-gbm"
    role: "analyzed"
    overlap: "unknown"
---
...
The ILP formulation is therefore a natural upstream complement to t078: instead of testing whether a pre-specified gene set is mutually exclusive, it discovers which gene sets are mutually exclusive. The recovered modules are data-driven analogs of the "hallmark-like units" discussed in `discussion:2026-06-07-hallmark-ordering-and-data-driven-modules` — and, because the partition is label-free, they do not presuppose the canonical hallmark boundary assignments.
...
- **Check q012 for update.** Add a note that the PLPM is relevant to the module-discovery sub-question raised in `discussion:2026-06-07-hallmark-ordering-and-data-driven-modules`: specifically, that it is a single-method alternative to the "t078 pairwise exclusivity → post hoc partition" approach, and that it lacks the observation-event correction required before deployment on diagnostic cohorts.
