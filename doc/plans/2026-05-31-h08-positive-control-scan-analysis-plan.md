...
skills_loaded:
  ...
  - id: data-genomics-somatic-mutation-qa
    reason: MC3 MAF input-call + denominator QA is prerequisite to treating the refit as verdict-bearing
  - id: data-expression-bulk-rnaseq-qa
    reason: PanCanAtlas RNA-seq is the substrate for the frozen NMF expression-module covariate set
  - id: statistics-compositional-data
    reason: per-sample exposures H are sum-constrained; prereg's suspicious-result plan requires a CLR/ILR or absolute-burden guard
  - id: statistics-power-floor-acknowledgement
    reason: realized per-arm effective n (post MC3∩clinical / MC3∩RNA-seq join) is unfilled and gates the read
  - id: statistics-sensitivity-arbitration
    reason: prereg pre-commits K±5, lung-pooling, and APOBEC-tissue-set sensitivity variants
---
...
> **locked** in `pre-registration:h08-positive-control` (committed 2026-05-30): three
...
- `hypothesis:h08` (prong H08a) — `method:h08-agnostic-association-model` — `pre-registration:h08-positive-control`.
- Activation gates named by the pre-reg: `task:t177` (**DONE** — prior-art scan landed) and
  `question:q018` (feasibility verdict — **OPEN, empty stub**).
