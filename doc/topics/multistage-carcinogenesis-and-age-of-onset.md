...
type: "topic"
title: "Multistage carcinogenesis and the determinants of differential age of cancer onset"
status: "active"
ontology_terms:
...
mutation supply alone. It is the background for `question:q041` (the prevalent-cohort test) and a
node in `theme:temporal-structure-of-carcinogenesis`.
...
- **`paper:Knudson1971`** — the cleanest single demonstration that **fewer required hits ->
  ...
  FAP *APC*). This is the conceptual root of `hypothesis:h06` (n-1 driver carriage).
...
- **Can (correlational only):** relate per-cancer-type median age at diagnosis to a
  mutation-derived driver-complexity proxy (`question:q041`), as a sanity check against
  `paper:Belikov2017`.
...
- **Theme:** `theme:temporal-structure-of-carcinogenesis` (order + count + hallmark-timing).
...
- **Discussion:** `discussion:2026-06-07-age-of-onset-and-multistage-mutation-requirement`.
