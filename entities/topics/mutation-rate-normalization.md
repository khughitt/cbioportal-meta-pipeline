...
rate correction is the difference between a 224-driver list from Lawrence et al. [@Lawrence2014]
after MutSigCV correction) and an ~17,000-gene list (genes with ≥5 mutations across the cohort, no
correction).
...
- **Trinucleotide context (192 classes)** captures sequence-context-dependent substitution rates
  (e.g., C>T at CpG vs C>T at non-CpG). Used by dNdScv (Martincorena et al. [@Martincorena2017]),
  the Chang et al. [@Chang2016] hotspot model, and COSMIC mutational signatures
  [@Alexandrov2020].
- **Genomic-region covariates.** MutSigCV (Lawrence et al. [@Lawrence2014]) uses three:
...
- **Per-codon background rate** (Chang et al. [@Chang2016]): combines trinucleotide context with
  gene-level
...
2. **Selection-based** (dNdScv, Martincorena et al. [@Martincorena2017]). Compares observed
   non-synonymous to
...
3. **Per-codon hotspot models** (Chang et al. [@Chang2016]). Codon-level recurrence with trinucleotide-
   adjusted background. Complementary to gene-level methods.

Field consensus in Bailey et al. [@Bailey2018]: combine multiple methods rather than rely on one. Their 26-tool
...
- **For panel data (most of our cohort), what's the best feasible background model?** dNdScv
  ...
  (Bailey et al. [@Bailey2018], CGC), not for *discovery* with a new background model.
